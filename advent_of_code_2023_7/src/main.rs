    ...
    for line in reader.lines() {
        match line {
            Ok(line) => {
                ...
                let joker_rank = get_card_rank_v2(&cards_count);
...
fn compare_hands_v2(hand1: &Hand, hand2: &Hand) -> Ordering {
    ...
    // if ranks are equal, compare cards from left to right
    for i in 0..5 {
        let dif = card_char_to_number_v2(hand1.cards.chars().nth(i).unwrap())
            - card_char_to_number_v2(hand2.cards.chars().nth(i).unwrap());
...
fn get_card_rank_v2(cards: &HashMap<char, u32>) -> u32 {
    // 7 Five of a kind, where all five cards have the same label: AAAAA
    // 6 Four of a kind, where four cards have the same label and one card has a different label: AA8AA
    // 5 Full house, where three cards have the same label, and the remaining two cards share a different label: 23332
    // 4 Three of a kind, where three cards have the same label, and the remaining two cards are each different from any other card in the hand: TTT98
    ...
    // 2 One pair, where two cards share one label, and the other three cards have a different label from the pair and each other: A23A4
    // 1 High card, where all cards' labels are distinct: 23456
    let jokers = cards.get(&'J').unwrap_or(&0).to_owned();
    ...
    // JJJ33 -> 7 Five of a kind
    // 22223
    // JJJJ3 -> 7 Five of a kind
...
mod tests {
    ...
    fn test_get_card_rank_v2() {
        let cards1 = count_chars("AAAAA");
        assert_eq!(get_card_rank_v2(&cards1), 7);

        let cards2 = count_chars("AA8AA");
        assert_eq!(get_card_rank_v2(&cards2), 6);

        let cards3 = count_chars("23332");
        assert_eq!(get_card_rank_v2(&cards3), 5);

        let cards4 = count_chars("TTT98");
        assert_eq!(get_card_rank_v2(&cards4), 4);

        let cards5 = count_chars("23432");
        assert_eq!(get_card_rank_v2(&cards5), 3);

        let cards6 = count_chars("A23A4");
        assert_eq!(get_card_rank_v2(&cards6), 2);

        let cards7 = count_chars("23456");
        assert_eq!(get_card_rank_v2(&cards7), 1);

        let cards8 = count_chars("222JJ");
        assert_eq!(get_card_rank_v2(&cards8), 7);

        let cards9 = count_chars("2222J");
        assert_eq!(get_card_rank_v2(&cards9), 7);

        let cards10 = count_chars("2223J");
        assert_eq!(get_card_rank_v2(&cards10), 6);

        let cards11 = count_chars("22JJ4");
        assert_eq!(get_card_rank_v2(&cards11), 6);

        let cards12 = count_chars("J3455");
        assert_eq!(get_card_rank_v2(&cards12), 4);

        let cards13 = count_chars("234JJ");
        assert_eq!(get_card_rank_v2(&cards13), 4);

        let cards14 = count_chars("23455");
        assert_eq!(get_card_rank_v2(&cards14), 2);

        let cards15 = count_chars("23456");
        assert_eq!(get_card_rank_v2(&cards15), 1);

        let cards16 = count_chars("2345J");
        assert_eq!(get_card_rank_v2(&cards16), 2);

        // 222JJ -> 7 Five of a kind
        let cards17 = count_chars("222JJ");
        assert_eq!(get_card_rank_v2(&cards17), 7);

        // 2222J -> 7 Five of a kind
        let cards18 = count_chars("2222J");
        assert_eq!(get_card_rank_v2(&cards18), 7);

        // 2223J -> 6 four of kind
        let cards19 = count_chars("2223J");
        assert_eq!(get_card_rank_v2(&cards19), 6);

        // 22JJ4 -> 6 four of kind
        let cards20 = count_chars("22JJ4");
        assert_eq!(get_card_rank_v2(&cards20), 6);

        // J3455 -> 3 two pair
        let cards21 = count_chars("J3455");
        assert_eq!(get_card_rank_v2(&cards21), 4);

        // 234JJ -> 4 three of a kind
        let cards22 = count_chars("234JJ");
        assert_eq!(get_card_rank_v2(&cards22), 4);

        // 2345J -> 2 one pair
        let cards23 = count_chars("2345J");
        assert_eq!(get_card_rank_v2(&cards23), 2);

        // KTJJT -> 6 four of a kind
        let cards24 = count_chars("KTJJT");
        assert_eq!(get_card_rank_v2(&cards24), 6);

        // QQQJA -> 6 four of a kind
        let cards25 = count_chars("QQQJA");
        assert_eq!(get_card_rank_v2(&cards25), 6);

        // T55J5 -> 6 four of a kind
        let cards26 = count_chars("T55J5");
        assert_eq!(get_card_rank_v2(&cards26), 6);

        // J5745 -> 4 Three of a kind
        let cards27 = count_chars("J5745");
        assert_eq!(get_card_rank_v2(&cards27), 4);
    }
}
